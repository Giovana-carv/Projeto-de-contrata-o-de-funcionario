...
<head>
  ...
  <!-- Bootstrap CSS apenas para toast -->
  ...
  <style>
    ...
    .log:hover {
      background: linear-gradient(45deg, #ff6b4b, #ff5a8c);
...
<body>
<div class="login-cadastro-container" id="container">
  ...
  <div class="form-container sign-up-container">
    <form id="formCadastro" enctype="multipart/form-data">
      ...
      <label class="custom-file-button" for="foto_perfil">Foto de Perfil</label>
      ...
      <div id="certificado-area" style="display:none;">
        ...
        <label class="custom-file-button" for="certificado">Selecionar Certificado</label>
        <input type="file" id="certificado" name="certificado" accept="image/*" />
      </div>
...
<div class="position-fixed top-0 end-0 p-3" style="z-index: 9999;">
  <div id="toastMsgBox" class="toast align-items-center text-bg-success border-0 shadow" role="alert" aria-live="assertive" aria-atomic="true">
    <div class="d-flex">
...
<script>
  ...
  signUpButton.addEventListener('click', () => containerEl.classList.add("right-panel-active"));
  signInButton.addEventListener('click', () => containerEl.classList.remove("right-panel-active"));
  ...
  function showToast(msg, isSuccess=true, delay=3000){
  ...
  document.getElementById('formCadastro').addEventListener('submit', function(e){
    ...
    fetch('../php/cadastro.php', { method:'POST', body: formData })
      .then(r => r.text())
      .then(txt => {
        if(txt.trim() === 'sucesso') {
          showToast('✅ Cadastro realizado com sucesso!', true, 2000);
          containerEl.classList.remove('right-panel-active');
          this.reset();
          certArea.style.display = 'none';
          camposFuncionario.style.display = 'none';
        } else {
          showToast('❌ ' + txt, false, 3500);
        }
      })
      .catch(err => showToast('❌ Erro de conexão. Tente novamente.', false, 3500));
  });
  ...
  document.getElementById('formLogin').addEventListener('submit', function(e){
    ...
    fetch('../php/login.php', { method:'POST', body: formData })
      ...
      .then(txt => {
        ...
        if(trimmed === 'sucesso') {
          showToast('✅ Login realizado com sucesso!', true, 2000);
          setTimeout(() => window.location.href = 'area_restrita.php', 2000);
        } else {
          showToast('❌ ' + trimmed, false, 3500);
        }
      })
      .catch(err => showToast('❌ Erro de conexão. Tente novamente.', false, 3500));
  });
